...
function Get-QwenRuntimeFacts {
    [CmdletBinding()]
    param(
        [string]$LocalRoot = (Get-ManifestedLocalRoot)
    )

    $definition = Get-ManifestedCommandDefinition -CommandName 'Initialize-QwenRuntime'
    if (-not $definition) {
        return (New-ManifestedRuntimeFacts -RuntimeName 'QwenRuntime' -CommandName 'Initialize-QwenRuntime' -RuntimeKind 'NpmCli' -LocalRoot $LocalRoot -Layout $null -PlatformSupported:$false -BlockedReason 'The packaged command definition for QwenRuntime is not available.')
    }
